...
import com.lorenzobraghetto.speakbird.Logic.TwitterServiceM;
...
public class SpeakServiceM extends Service {
	private static SyncAdapterImpl sSyncAdapter = null;
 
public SpeakServiceM() {
	 super();
 }
...
private static class SyncAdapterImpl extends AbstractThreadedSyncAdapter {
	...
	public SyncAdapterImpl(Context context) {
		...
		ContentResolver.addPeriodicSync(account, "com.lorenzobraghetto.speakbird.contentm", new Bundle(), Long.parseLong(time)*60);
	}
 
	@Override
	public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
		try {
			SpeakServiceM.performSync(mContext, account, extras, authority, provider, syncResult);
...
private SyncAdapterImpl getSyncAdapter() {
 ...
 private static void performSync(Context context, Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult)
	throws OperationCanceledException {		
		Intent intent = new Intent(context, TwitterServiceM.class);
