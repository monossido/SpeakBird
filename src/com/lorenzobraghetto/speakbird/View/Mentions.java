...
public class Mentions extends SherlockActivity implements OnInitListener, OnUtteranceCompletedListener
{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if(isNetworkAvailable())
			...
			Toast.makeText(getApplicationContext(), "Connection not avaible", Toast.LENGTH_SHORT).show();
        ...
        case R.id.controlli:
			switch(controls) {
	            case 0:  
	            	...
	            	controls=1;
	    			Toast.makeText(getApplicationContext(), R.string.controllerAll, Toast.LENGTH_SHORT).show();

	            	break;
	            case 1:
	            	item.setIcon(R.drawable.controlliuno);
	            	controls=0;
	    			Toast.makeText(getApplicationContext(), R.string.controller, Toast.LENGTH_SHORT).show();
			}
        	return true;
        case R.id.pause:
        	speaking=false;
	...
	private class mentionsProgress extends AsyncTask<Void, Void, Boolean>
	{
		...
		protected void onPreExecute()
		{
			dialogP = ProgressDialog.show(Mentions.this, "", 
		...
		protected Boolean doInBackground(Void... params) {
	        ...
	        adapter=new MentionsAdapter(
	        			mContext,
	                        data,
	                        mentions.get(0).getCreatedAt().getTime(), -1);
		...
		protected void onPostExecute(Boolean result)
		{
			if(result)
			{
				...
				list = (PullToRefreshListView) findViewById(R.id.mentionsListView);
	...
	private final OnItemClickListener onClickMention = new OnItemClickListener()
	{
	...
	private void reDrawList(int position)
	{
		ArrayList<Object> data=new ArrayList<Object>();
        ...
        adapter=new MentionsAdapter(
    			mContext,
                    data,
                    mentions.get(0).getCreatedAt().getTime(), position);
    ...
    private Locale getLocaleMentions()
    {
